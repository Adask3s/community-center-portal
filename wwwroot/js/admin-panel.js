...
async function zapiszWiersz(button) {
  ...
  row.querySelectorAll("[data-field]").forEach((cell) => {
    ...
    if (
      ["KategoriaId", "InstruktorId", "MaksymalnaLiczbaUczestnikow"].includes(
        field,
      )
    ) {
  ...
  const url = id
    ? `/AdminApi/Update/${entityType}/${id}`
    : `/AdminApi/Create/${entityType}`;
...
function dodajNowyWiersz(entityType) {
  const table = document.querySelector(
    `table[data-type='${entityType}'] tbody`,
  );
