...
namespace DomKultury.Controllers
{
    ...
    public class ZajeciaController : Controller
    {
        ...
        private readonly ILogger<ZajeciaController> _logger;

        public ZajeciaController(WydarzeniaContext context, IWebHostEnvironment env, UserManager<IdentityUser> userManager, ILogger<ZajeciaController> logger)
        {
            _context = context;
            _env = env;
        ...
        public async Task<IActionResult> Create(Zajecie model, IFormFile Obrazek)
        {
            ...
            foreach (var entry in ModelState)
            {
                ...
                {
                    _logger.LogError($"Błąd w polu {entry.Key}: {error.ErrorMessage}");
                }
            }
